...
randmf_from_df<-function(df_list, ord="circular6"){
  ...
  if(nmat <2){
    stop("Expect two or dataframes to be input in list")
  }
  ...
  for (d in df_list){
    ...
    # janitor::round_half_up(za, digits = 4)
  }
  ...
  if(nper<50000)permat<-permute::allPerms(n, control = permute::how(maxperm = 500000), check = TRUE)
